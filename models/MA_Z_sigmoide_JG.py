...
def custom_loss2(K,R):
  ...
  def max_like(y_true, y_pred): # y_true [batch_size, K, R]
    #kernels##############################################
    yhat1 = tf.nn.softmax(y_pred[:,:K])
    Z = tf.math.sigmoid(y_pred[:,K:])
    N = y_true.shape[0]
    #K = y_pred[:, R:].shape[1]
    #K = np.unique(y_true).size
    y_hat = tf.repeat(tf.expand_dims(yhat1,-1), R, axis = -1)
...
class Keras_MA_pi_kern(BaseEstimator, TransformerMixin):
  ...
  def fit(self, X, t):
    ...
    hW = tf.keras.layers.Dense(Q1+Q2,activation='linear',name='Y',  bias_initializer='zeros', kernel_initializer=initializer,
  ...
  def score2(self, X, t):
    ...
    #K = y_pred[:, R:].shape[1]
    #K = np.unique(y_true).size
    y_hat = np.repeat(np.expand_dims(y_pred[:, :self.K],-1), self.R, axis = -1)

    Z = y_pred[:, self.K:]

    p_logreg = np.prod(np.power(y_hat, y_true), axis=1)
      
    #Likelihood 

    temp1 = Z*np.log(p_logreg)       
    temp2 = (1-Z)*np.log(1/self.K)   

    #####funcion de costo############################################
    f = np.sum(temp1+temp2)
